define(["data/scoreobject"],function(scoreObject){
	...
	var titleProgress = {
		...
		cols:[
			{
				"template": "<span class='webix_icon fa-adjust'></span>店员T分进度", "css": "sub_title2", borderless: true
			},
//			{ view: "button", type: "iconButton", icon: "sliders", label: "Update", width: 100}
	...
	return { 
			$ui: layout,
			$fnInvoke:function(DeptCode,YearMonth){
				
			var condition = 
			{
				DeptCode:DeptCode,
				YearMonth:YearMonth,
				RankAtt:'StaffGARatio',
				PageIndex:1,
				PageLen:1000
			}
			
			$$("storestafftscore").clearAll();
			$$("storestafftscore").parse(scoreObject.getRanker(condition));
		}
	};
