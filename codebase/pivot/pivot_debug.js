...
webix.protoUI({
			...
			for(var i=0; i < columns.length && order===null; i++){
				...
				if(typeof c == "object" && c.sort){
					...
					if(typeof c.sort == "string")
						sortFunction = webix.DataStore.prototype._sort._as[c.sort];
					var res = sortFunction(a[i].text,b[i].text);
		...
		for (var i = 0; i < structure._header.length; i++) {
			...
			if (item[key]){
				...
				for(var j=0; j < item[key].length;j++){
					if(item[key][j] || item[key][j]=="0"){
						data.push(item[key][j]);
					}
				}
				if(data.length)
					item[key] = this.operations[op].call(this, data);
				else
					item[key] = '';
			}
			else
	...
	getFields: function() {
		...
		for (var i = 0; i < str.columns.length; i++) {
			var field = (typeof str.columns[i] == "object"? (str.columns[i].id||i): str.columns[i]);
			if (!webix.isUndefined(fields_hash[field])) {
